...
static bool mask_status(W25q *flash, const uint8_t cmd, uint8_t mask)
{
...
bool W25qPageWrite(W25q *flash, size_t address, uint8_t *data, size_t size)
{
    ...
    // Write enable instruction 0x6, Prog Page 0x2 + 24 bit address
    uint8_t txbuf[5] = {0x6, 0x2, address >> 16, (address >> 8) & 0xFF, address & 0xFF};
    uint8_t rxbuf[256] = {0};
    flash->bus->send(flash->bus, txbuf, 5);
...
bool W25qSectorErase(W25q *flash, size_t address)
{
    ...
    uint8_t txbuf[5] = {0x6, 0x20, address >> 16, (address >> 8) & 0xFF, address & 0xFF};
    flash->bus->send(flash->bus, txbuf, 5);
