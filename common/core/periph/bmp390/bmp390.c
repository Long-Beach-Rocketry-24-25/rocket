...
static void Bmp390_Get_Calib(Bmp390* dev)
{
    ...
    dev->status = dev->bus->read(dev->bus, BMP390_CALIB_REG_START, data,
                                 BMP390_CALIB_REG_LEN);
...
void Bmp390_Config(Bmp390* dev)
{
    ...
    uint8_t conf = BMP390_PWR_CTRL_MODE | BMP390_PWR_CTRL_PRESS_EN |
...
static int64_t Bmp390_Compensate_Temperature(Bmp390* dev, uint32_t data)
{
    ...
    partial_data4 =
        (int64_t)(((int64_t)partial_data3) * ((int64_t)dev->cal.t3));
    partial_data5 =
        ((int64_t)(((int64_t)partial_data2) * 262144) + (int64_t)partial_data4);
...
static int64_t Bmp390_Compensate_Pressure(Bmp390* dev, uint32_t data)
{
    ...
    offset = (int64_t)((int64_t)(dev->cal.p5) * (int64_t)140737488355328U) +
    ...
    partial_data5 =
        ((int64_t)(dev->cal.p2) - 16384) * ((int64_t)dev->cal.t_fine) * 2097152;
    sensitivity =
        (((int64_t)(dev->cal.p1) - 16384) * (int64_t)70368744177664U) +
    ...
    partial_data2 =
        ((int64_t)(dev->cal.p11) * (int64_t)(partial_data6)) / 65536;
...
static void Bmp390_Get_Data(Bmp390* dev)
{
    ...
    dev->status =
        dev->bus->read(dev->bus, BMP390_TEMP_DAT_REG_START, &data[3], 3);
